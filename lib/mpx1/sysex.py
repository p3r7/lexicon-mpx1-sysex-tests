...
def device_inquiry(inport, outport, device_id = 0x7f, sync=True):
    ...
    if sync:
        ...
        rcv = inport.receive()
        ic(rcv.hex())
        parse_device_inquiry(rcv.bytes())

def parse_device_inquiry(rcv_bytes):
    return {
...
def set_current_program(outport, v, device_id = 0x7f, sync=True):
    set_param_data(outport, [0, 19, 9], 2, v, 'int', device_id, sync)
...
def get_param_data(inport, outport, control_levels, value_type = 'int', device_id = 0x7f, sync=True):
    rcv = request_by_control_levels(inport, outport, 1, control_levels, device_id, sync)
    if sync:
        rcv = rcv.bytes()
...
def get_param_display(inport, outport, control_levels, device_id = 0x7f, sync=True):
    rcv = request_by_control_levels(inport, outport, 2, control_levels, device_id, sync)
    if sync:
        rcv = rcv.bytes()
        size_bytes = unnibblize(rcv[5:9])
...
def get_param_desc(inport, outport, param_type, device_id = 0x7f, sync=True):
    rcv = request_by_param_type(inport, outport, 4, param_type, device_id, sync)
    if sync:
        rcv = rcv.bytes()
